...
Use Node.js 24, run `npm ci` and `npm run dev`, and open `http://127.0.0.1:5180/`. The same editor opens at `/index.html`. For production, run `npm run build` then `npm start` on port 5180.
...
`VITE_LUMEN_TILEJSON_URL` can select a compatible OpenMapTiles TileJSON URL at build time. The default is `https://tiles.openfreemap.org/planet`. This source supplies transportation, water, landuse, landcover, park and flat building footprints. Its building heights are not used. Public service availability is not guaranteed; deployments can self-host compatible tiles. No public Overpass endpoint is used for continuous panning. Search uses the inherited bounded gateway.

The global building service supplies ordinary buildings everywhere; attributed snapshots retain road/rail topology and place labels. Reviewed landmark models retain their geometry. Guangzhou, Kaohsiung, Yokohama and Lower Manhattan use global vector tiles, without new bundled road or rail snapshots. Buildings use raw Overture heights/floors, automatic geographic enrichment and marked estimates, and roads use quantized coordinate junctions. This loses tags and original OSM node identity. Global roof shapes, building relations, lanes and grade-separated junctions are not fully represented. Roads and cars at the boundary of a bundled snapshot are not yet a single persistent transport network. Global trains are not implemented.

The renderer requires global tiles even for preset scenes. Network failure is surfaced with Retry; a completely offline preset ground layer remains future work. Preset ground artwork adds contact shadows and soft road lighting. Water, parks, woods, grass and bridge masks are decoded from separate zoom-14 environment tiles. Polygon holes retain islands and clearings. A packed coverage texture keeps vegetation clear of roads, water and buildings. Mapped woodland receives instanced low-polygon crowns and trunks; parks receive occasional inferred groves, while explicit grass remains open. Individual tree positions are artistic estimates. Trees share one draw call and appear at neighbourhood zoom levels.
...
The [global building service](GLOBAL-BUILDING-SERVICE.md) is required for every location. Its version 2 manifest supplies one tile URL template for the entire supported grid; regional manifests are rejected. Nullable Overture heights/floors and their sources are preserved. Background GHS-BUILT-H and covered PLATEAU/EUBUCCO sources can improve marked estimates. A service failure surfaces Retry instead of substituting lossy basemap building heights. See [height selection](BUILDING-HEIGHTS.md).
...
Building delivery and background updates are described in [Global building service](GLOBAL-BUILDING-SERVICE.md). There is no optional regional building path.