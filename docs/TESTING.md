...
For building heights, run `python -m unittest discover -s scripts/buildings -p test_buildings.py` in the preprocessing environment. Node tests cover nullable/unknown provenance, geometry heights, bounded downloads, tile deduplication, mandatory global delivery, rejected regional manifests and aborts. `node scripts/check-3d-heights.mjs` runs against the default global service; `QA_HEIGHT_CENTER` selects any longitude/latitude with building data. It checks desktop/mobile layouts in both languages, real PNG capture, source credits, automatic loading in Africa and atlas disposal. Inspect the resulting images. See [building data](BUILDING-HEIGHTS.md).
