...
test('orthographic ground and height form one camera with orthonormal screen axes',()=>{
  for(const elevation of [60,70,80]) for(const azimuth of [undefined,0]) {
    const {matrix,direction}=sceneProjection({projection:2,elevation,azimuth});
    const z=transformPoint(direction,matrix),[a,b,c,d]=matrix;
    if(azimuth===0) {assert.equal(z[0],0);assert(z[1]<0);}
...
test('projected wallpaper corners remain inside mapped ground in portrait and landscape',()=>{
  for(const azimuth of [undefined,0]) {
    const matrix=sceneProjection({projection:2,elevation:60,azimuth}).matrix,bounds=[-800,-900,800,900];
