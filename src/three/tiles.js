...
import { boundedBytes, parseHeightTile, validateManifest } from "./height-tiles.js";
    ...
    for (
      ...
      const results = await Promise.allSettled(
        batch.map(async (t) => {
          ...
          const source = this.heightMetadata.tiles.replace("{z}", t.z).replace("{x}", t.x).replace("{y}", t.y);
      ...
      for (const [index, t] of batch.entries()) {
        ...
        for (const credit of info?.attribution || this.heightMetadata.attribution) credits.add(credit);
