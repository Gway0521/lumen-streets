...
export function outputPlan(source: FrameView, size: ExportSize, format: ExportFormat, resolution: number = 1080) {
  ...
  let [width, height] = size === "desktop" ? [3840, 2160] : size === "desktop1610" ? [3840, 2400] : size === "portrait" ? [1080, 1920] :
    [Math.round(source.width * (source.dpr ?? 1)), Math.round(source.height * (source.dpr ?? 1))];
  if (format === "gif") {
  ...
  // Preserve fractional DPR rounding exactly for the existing current-view PNG path.
  const view = size === "current" && format === "png" ? structuredClone(source) : cropView(source, width, height);
...
export function wallpaperPlan(source: FrameView, bounds: readonly number[], camera: Camera, size: ExportSize, format: ExportFormat, resolution: number = 1080) {
  const plan = outputPlan(source, size, format, resolution);
